	...
	c.prototype.initToolbar = function() {
		if(this.showToolbar = this.options.showExport, d.apply(this, Array.prototype.slice.apply(arguments)), this.options.showExport) {
			var c = this,
				...
				f = e.find("div.export");
			if(!f.length) {
				f = a(['<div class="export btn-group" title="导出">', '<button class="btn btn-default dropdown-toggle" data-toggle="dropdown" type="button">', '<i class="iconfont icon-export"></i> ', '<span class="caret"></span>', "</button>", '<ul class="dropdown-menu" role="menu">', "</ul>", "</div>"].join("")).appendTo(e);
				...
				a.each(h, function(a, c) {
					...
					if("all" === c.options.exportDataType && c.options.pagination) c.togglePagination(), c.$el.on("load-success.bs.table", function() {
						d(), c.$el.off("load-success.bs.table"), c.togglePagination()
					});
					else if("selected" === c.options.exportDataType) {
						var e = c.getData(),
							f = c.getAllSelections();
						c.load(f), d(), c.load(e)
					} else d()
				})
			}
